...
The official `@n8n/scan-community-package` 0.34.0 scanner accepts only a published npm package, verifies npm provenance, and retrieves the attested public source. An unpublished package or private source repository therefore cannot receive a meaningful pre-publication scanner result. The tag workflow runs `npm run scan:published` after publication and requires the scanner's explicit success text before Creator Portal submission; scanner exit status alone is insufficient.
