...
- `tests/search-metrics-traces.live.test.ts`: guarded loopback-only Search/Metric flow, including JSON rows plus CSV and Markdown query output, and honest empty/missing Trace behavior. Test-only direct API calls seed metric records and the empty trace fixture without exposing those deferred operations in the node. It creates three exact run-scoped streams, deletes each with `delete_all=false`, and confirms absence in `finally`.
...
Version 0.1.2 is the current published release. The validation baseline established for the 0.1 release line uses OpenObserve OSS v0.92.2 at the immutable image digest above. The independent live suites cover Stream management and Log ingestion; SQL Search and formatted output; all advertised Prometheus reads against test-only directly seeded metrics; honest empty/missing Trace reads; Function/VRL and Dashboard lifecycle; Alert Template/Destination/Alert lifecycle plus real webhook delivery; Trigger activation/reconciliation/delivery/deactivation; and real-time/scheduled Pipeline lifecycle including real stream forwarding. Trace data is not seeded through an out-of-scope ingestion API. Alert and Pipeline history are exercised at their strongest pinned behavior: v2 Alert History returns a valid empty wrapper; old Pipeline History servers can return a specific missing-`triggers` 500, which the node narrowly treats as empty only after an explicit Pipeline selection. Current upstream already returns an empty Pipeline History wrapper for this condition. Cloud runtime behavior is not claimed.

`npm run scan:source` provides the official scanner 0.34.0 source and built-artifact preflight before publication. Registry provenance and attested public source can only be checked after a version exists on npm, so a separate `verify-published` job runs `npm run scan:published` after `publish` succeeds. GitHub's **Re-run failed jobs** can therefore retry propagation verification without republishing the immutable version; never rerun a successful publish job. Explicit scanner success text is required. Bounded retries are limited to the exact observed newly published-version metadata absence, analysis 404, and provenance source-repository 404. A 403, scanner policy/lint finding, timeout, rate limit, or arbitrary failure is immediate.
...
A disposable n8n 2.37.10 headless-Chrome run measured authenticated sign-in/editor loading at 2.387 seconds and new-workflow readiness at a further 2.802 seconds on this workstation. It observed zero dynamic-node-parameter requests before a locator was opened. The headless node-picker layout did not expose a reliable search control, so resource/operation transition timings and locator-open request counts are not claimed from browser evidence.

Metadata inspection found the concrete editor problem: nine resource description helpers emitted an unsupported top-level `show` property instead of `displayOptions.show`. n8n therefore rendered operation-specific controls without their intended resource/operation conditions, producing the reported long forms. The helpers now emit valid display conditions, and automated checks cover representative Stream Get Many, Metric Instant/Range Query, Alert Create, and Pipeline Create selections, reject any top-level `show`, and verify unrelated dynamic locators remain hidden. Dynamic list methods are invoked by locator controls rather than during node selection; no caching or credential-sensitive behavior changed.

## Core E2E scenario

The implemented suites authenticate; ingest timestamped JSON logs; confirm inferred streams/schema; query with SQL/date/offset/limit; retrieve field values and search around; query test-only seeded metrics with PromQL; validate and create a VRL function; exercise Dashboard, Alert, and Pipeline lifecycles; and delete only exact-owned resources. Trace reads run separately. Trigger coverage proves secret-authenticated receipt, selected-alert attachment, activation rollback, restart reconciliation, and safe shared-artifact cleanup. It does not claim replay suppression: OpenObserve retries may yield duplicate executions, so downstream exactly-once effects require workflow-level deduplication/idempotency.
